    ...
    def get_data(nama_sheet):
        try:
            ...
            if nama_sheet == "siswa":
                ...
                # Menambahkan kolom prodi di dataframe rekap jika sheet belum ada
    ...
    if menu == "Input Absensi":
        ...
        if df_siswa.empty:
            ...
            for i, row in df_filtered.iterrows():
                ...
                # SEKARANG PRODI IKUT DISIMPAN KE LIST
            ...
            if st.button("SIMPAN SEMUA DATA PRODI INI", type="primary", use_container_width=True):
                with st.spinner("Sedang menyimpan ke Google Sheets..."):
                    ...
                    st.success(f"Berhasil Simpan Absensi & Prodi untuk {len(list_input)} Siswa!")

    elif menu == "Monitoring":
        st.header("📊 Hasil Absensi")
        df_rekap = get_data("rekap")
        
        # Filter Monitoring menggunakan kolom prodi yang sudah ada di rekap
        if not df_rekap.empty and 'prodi' in df_rekap.columns:
            daftar_prodi_rekap = ["Semua"] + sorted(df_rekap['prodi'].unique().tolist())
            prodi_monitor = st.selectbox("Filter Tampilan Prodi:", daftar_prodi_rekap)
            
            if prodi_monitor != "Semua":
                df_tampil = df_rekap[df_rekap['prodi'] == prodi_monitor]
